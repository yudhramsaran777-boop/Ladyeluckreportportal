...
## ⛔ STATUS: DISCONTINUED — 2026-06-24

**The Gmail / live-payment-activity system described in this blueprint was
discontinued by owner decision on 2026-06-24. This document is retained for
historical reference only.**

### What was removed

All frontend code, API routes, server actions, components, hooks, and
server-side utilities that formed the Gmail ingestion and live payment
dashboard have been deleted from the codebase (migration 0023,
git branch `feature/payment-dashboard`).

### What remains active

- Original CashApp and Chime payment account management (manager CRUD).
- Employee Payment Info page showing active CashApp / Chime carousels,
  account credentials, QR images, tags, and payment links.
- All shop, employee, manager, shift-report, cashout, and game-account
  functionality — completely unchanged.

### Feature flags

All three payment-system feature flags have been permanently set to `false`
by migration `0023_disable_payment_activity_system.sql`:
...
**Do not set any of these flags to `true` without a new explicit owner
decision and a full re-implementation review.**

### Database tables

The following tables are **retained temporarily** for safe rollback and
audit history. Browser (anon/authenticated) access has been revoked.
Do not drop these tables unless explicitly requested after the portal
...
To permanently drop them, create a new destructive migration only after
confirming no rollback is needed.

### Environment variables that may be removed from Vercel

The following environment variables were used exclusively by the
discontinued system. They can be removed from Vercel project settings:
...
**Do not remove** `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`,
or `SUPABASE_SERVICE_ROLE_KEY` — those are used by the entire portal.
